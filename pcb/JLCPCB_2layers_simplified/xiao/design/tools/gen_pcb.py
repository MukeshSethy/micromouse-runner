...
class PcbGen:
    ...
    def add_keepout(self, rect, allow_tracks=False, allow_footprints=False, allow_vias=None, layer=None):
        ...
        # `layer` (added for the XIAO antenna keepout, which must exclude
        # copper POUR on both F.Cu and B.Cu, not just the default F.Cu) lets a
        # caller target a specific layer; call twice (once per layer) for a
        # both-sides exclusion.
        x1, y1, x2, y2 = rect
        z = pcbnew.ZONE(self.board)
        z.SetLayer(layer if layer is not None else pcbnew.F_Cu)
    ...
    def _verify_geo(self, segments, via_points, net_name, half_w, verify_clr=0.16,
                     edge_margin=0.7, verify_clr_tht=0.5):
        # verify_clr_tht (DEFAULT 0.5, bumped from 0.3 per user request for
        # easier hand-soldering room around THT pads): THT (holed) pads always keep the full
