...
# FLAG FOR REVIEW (line-follower revision): QRE1113GR (Q41-Q46, custom
# footprint, B.Cu) has no JLC-verified rotation_offset entry -- this is a
# brand-new project-local footprint with no prior JLC assembly-preview check
# (unlike every correction above, each derived from a real preview/placement
# check on an earlier board). Re-verify Q41-Q46's orientation in JLCPCB's own
# assembly-preview tool before ordering; add a ROTATION_CORRECTIONS entry for
# "^QRE1113GR" if the preview shows it mis-rotated.
