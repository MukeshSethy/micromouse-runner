...
# ---------------------------------------------------------------------------
# LINE-FOLLOWER FRONT SENSOR ARRAY (this project only, forked from xiao/):
# 6x QRE1113GR on B.Cu (flip=True -- faces straight down at the floor),
# along the board's flat front-edge span. BOARD_OUTLINE's front corners are
# CORNER_R=16mm smooth curves starting at x=16 and x=84 (see board_geom.py),
# so x16-84 at low y is the only straight front edge available -- 8mm pitch
# centered on CX (matches Pololu's own QTR-MD-06RC 6-channel/8mm-pitch
# reference design) spans x30-70, comfortably inside that window.
#
# Y=8 (not right at y=0): the front castor mounting hole sits at (CX,4)
# r=1.5mm (MOUNT_HOLES below), and the existing WALL-FRONT sensors Q2/Q3
# (real courtyard queried via pcbnew: y11.7-18.3) start right below y=11.7
# -- rot=90 turns each sensor's long body axis vertical (courtyard becomes
# ~4.4mm tall x ~2.2mm half-width in X) so adjacent 8mm-pitch sensors clear
# each other, and y=8 keeps the rotated courtyard (y5.1-10.9) clear of both
# the castor (y2.5-5.5) and Q2/Q3 (y>=11.7) with margin either side.
#
# Support resistors (R91-R102) are NOT crammed into this same tight nose
# strip -- at 8mm pitch there isn't room for 2 more parts per channel beside
# an already-tight sensor row without colliding with Q2/Q3 or the neighbour
# channel's own resistors (both caught by a real courtyard-overlap DRC pass
# during development). They add no optical/aiming constraint (unlike the
# sensor itself), so they're grouped instead in the open area beside the new
# ADS7830 (U4) at the board's rear, where they electrically connect anyway
# -- short traces to a nearby ADC pin, not a tight physical requirement.
# Indicator LEDs DO stay directly above their own sensor (F.Cu, same X),
# per user request -- their courtyard is small (0603) and clears both
# the sensor and Q2/Q3 easily at this pitch.
# ---------------------------------------------------------------------------
LINE2_PITCH = 8.0
LINE2_X0 = CX - 2.5 * LINE2_PITCH
LINE2_Y = 8.0
for i in range(6):
    x = LINE2_X0 + i * LINE2_PITCH
    g.place(f"Q{41 + i}", x, LINE2_Y, rot=90, flip=True)   # QRE1113GR (B.Cu, faces the floor)
    g.place(f"D{41 + i}", x, LINE2_Y - 3.5)                # indicator LED (F.Cu, directly above)
print(f"Line-follower sensor array: 6x QRE1113GR at x={[round(LINE2_X0 + i*LINE2_PITCH,1) for i in range(6)]}, y={LINE2_Y} (B.Cu)")

# Line-sensor emitter current-limit resistors (220R, 6 total -- the ADC
# series/isolation resistor per channel was REMOVED from the schematic:
# routing this design showed the extra 6 resistors added real congestion
# for no electrical benefit, and a bare collector-to-ADC-input wire is both
# standard practice AND strictly more "minimal" per the user's own
# requirement). Grouped beside the new ADS7830 (U4), NOT the crowded front
# nose (see note above). Real constraints queried via pcbnew: the LEFT
# WHEEL NOTCH removes board material at x0-13,y68-100 (FACE_L=13, AXLE_Y+-
# 16 -- NOT the 48/80 the board_geom.py comment states, which is stale; the
# real computed/loaded values are 68/100, confirmed against the actual
# Edge.Cuts geometry), and SW6/SW7 occupy x1.6-19.9,y113.9-119.8. That
# leaves a clear strip at x14-20,y100-113.9 -- a single column, 2.4mm row
# pitch (real R_0805 courtyard queried via pcbnew: 2.0mm tall), fits all 6
# with margin.
# Ref numbering: removing the ADC-isolation resistor made ref("R") allocate
# these SEQUENTIALLY (R91-R96), not every-other (R91,93,95..) like the old
# 2-per-channel scheme -- confirmed against the regenerated netlist (a first
# attempt at the old 91+2*i formula left R92/94/96 as netlist->pad mapping
# failures, since those ref numbers now belong to DIFFERENT channels' D/Q
# nets than before).
for i in range(6):
    ry = 100.3 + i * 2.4
    g.place(f"R{91 + i}", 15, ry)   # IR emitter current-limit (220R)
...
for ref, fx, rot in (("MOT1", FACE_L, 180), ("MOT2", FACE_R, 0)):
    ...
    # Line-follower revision: the footprint now carries a SECOND model too
    # (N20_Wheel.wrl, added for viewing/clearance-check per user request --
    # no real vendor STEP exists for a specific N20-compatible wheel, so it
    # stays VRML-only, same limitation as QRE1113GR). The rewrite below must
    # only touch the motor BODY model (N20_Motor_Encoder.wrl -> its real
    # .step counterpart) -- a blind "any .wrl" rewrite would also redirect
    # the wheel to "N20_Wheel.step", which doesn't exist, silently dropping
    # it from the 3D view on the next regeneration.
    _mot_models = fp.Models()
    for _i in range(_mot_models.size()):
        _m3d = _mot_models[_i]
        if _m3d.m_Filename.endswith("N20_Motor_Encoder.wrl"):
...
# Buttons REMOVED (line-follower revision, user's confirmed choice): SW1/SW2/
# R10 are gone (dead refs, no-op via _safe_place -- the schematic no longer
# has them). Their old spot (x21-32, y104-113) is reused below for the new
# ADS7830 I2C ADC + its pull-ups/decoupling, since it's already the closest
# open area to the XIAO module's pads 22/23 (now SDA/SCL) that fed them.
#
# ADS7830 (U4): TSSOP-16, in the gap between SW7 (real courtyard queried via
# pcbnew: x11.1-19.9, y113.9-119.8) and U3/the XIAO module (courtyard
# x41.1-58.9, y99.1-120.1) -- x21-40 is clear of both. Pads 22/23 (now
# SDA/SCL) sit at board-absolute (41.3,105.7)/(41.3,108.3) -- confirmed via
# pcbnew pad query -- so this spot keeps the new I2C routing short.
#
# Shifted from x=30 (real routing pass caught it): U4's own courtyard
# reached to x=27.2, only ~0.3mm from the PRE-EXISTING D30<->R82 (PWRLED_A)
# pad column at x~26.9. Shifted to x=33 (still not enough) -- a real DRC
# pass then caught a much worse problem: D33/R84 (MOTLED_A indicator,
# placed LATER in this file -- after the courtyard-overlap gate above
# already ran, so that gate never saw them) sit at x~30.3-33.7, and x=33
# put U4's own courtyard (30.2-35.8) almost entirely on top of them --
# confirmed via real DRC shorting_items output (GND<->MOTLED_A, SDA<->
# VM_6V, PLUS3V3<->VM_6V, MOTLED_A<->LINE3/4_ADC -- U4 and D33/R84's PADS
# were physically touching). x=37.5 clears D30/R82 (x<=27.73), D33/R84
# (x<=33.73), and U3 (x>=41.05) all with real margin -- verified directly
# via pcbnew courtyard bbox query, not just this script's own gate, since
# that gate is proven blind to late-placed refs.
g.place("U4", 37.5, 108, rot=90)
g.place("R103", 23, 102)     # SDA pull-up (2.2k) -- clears SW7 (x<=19.9) and the line-sensor resistor grid (x<=19)
g.place("R104", 38.5, 102)   # SCL pull-up (2.2k) -- real courtyard query caught x=39.5 overlapping U3 by 0.18mm; 38.5 clears it
g.place("C40", 23, 115)      # VDD decoupling (100nF)
g.place("C41", 38.5, 115)    # REFin/REFout decoupling (100nF)
...
# ---------------------------------------------------------------------------
# LINE-FOLLOWER REVISION: move a broad set of existing F.Cu-only passives to
# B.Cu (user's explicit permission + suggestion: "you can take many other
# smd's to bottom layer also"). Before this, 104 of 110 footprints sat on
# F.Cu (only the 6 new QRE1113 sensors were on B.Cu) -- a real routing pass
# showed this left many nets (motor/encoder signals, the power-divider
# section, wall-sensor support passives) unroutable-by-congestion, not
# fixable by re-ordering or relaxing clearance (both tried and plateaued).
# Selected: pure R/C passives only (no ICs/connectors/switches, which carry
# silkscreen-legend/mechanical assumptions this quick pass didn't re-verify)
# in the specific areas the stuck nets cluster around -- wall-sensor support
# (R13-24, R62-64), the power-divider section (R69-71/R73-74/R85, C15-C18),
# and the encoder pull-ups (R6-9). GND is poured on BOTH F.Cu and B.Cu
# already, so flipping loses no ground reference.
_FLIP_TO_BOTTOM = ["R13", "R14", "R15", "R16", "R17", "R18", "R19", "R20", "R21", "R22", "R23", "R24",
                   "R62", "R63", "R64", "R69", "R70", "R71", "R73", "R74", "R85",
                   "C15", "C16", "C17", "C18", "R6", "R7", "R8", "R9"]
_n_flipped = 0
for _ref in _FLIP_TO_BOTTOM:
    _fp = g._placed.get(_ref)
    if _fp is not None and not _fp.IsFlipped():
        _fp.Flip(_fp.GetPosition(), pcbnew.FLIP_DIRECTION_LEFT_RIGHT)
        _n_flipped += 1
print(f"Flipped {_n_flipped}/{len(_FLIP_TO_BOTTOM)} passives to B.Cu for routing headroom "
      f"(F.Cu now {len(g._placed) - 6 - _n_flipped}, B.Cu now {6 + _n_flipped})")
