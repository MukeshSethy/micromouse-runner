...
OUT = os.path.normpath(os.path.join(BASE, "..", "production_2layer", "jlcpcb"))
ZIP = os.path.join(OUT, "micromouse-pcb-simplified-2L-jlcpcb-gerbers.zip")
...
for r in rows:
    ...
    for x in refs:
        if "-" in x:
            range_bad.append(x)
    if not r[3].strip():
        # documented exceptions: the XIAO is deliberately hand-solder (NOT in
