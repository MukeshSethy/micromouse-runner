...
def _bom_fields(ref, value, footprint):
    ...
    if fp == "QRE1113GR":
        return {"MPN": "QRE1113GR", "Manufacturer": "onsemi"}
...
#   pad22 Plus8 SDA (was USER_BTN)   pad23 Plus9 SCL (was USER_BTN2)
#
# LINE-FOLLOWER REVISION (this project only, forked from xiao/): both onboard
# user buttons (SW1/SW2, pads 22/23) are REMOVED to free the only 2 spare
# GPIO pins on this module's exact 20/20 budget -- nRF52840 has no fixed
# hardware I2C pins (any 2 GPIOs can be TWI SDA/SCL via firmware pin-select),
# so pads 22/23 become SDA/SCL feeding a new ADS7830 (I2C, 8-ch, 8-bit ADC)
# that reads the 6 new front line sensors. User's explicit choice (of 3
# offered options) when asked which 2 existing signals to sacrifice.
...
# counter used by every wire between here and there (confirmed the hard way
# -- adding this section here shifted LINE4/5_ADC's bend positions onto each
# other, an ERC "multiple_net_names ... same items" short, exactly the
# failure mode the buzzer comment already documents).
...
# 8x QRE1113GR (onsemi SMT reflective object sensor -- the same part used
# inside Pololu's own QTR reflectance sensor arrays, confirmed against
# Pololu's own hosted datasheet copy at pololu.com/file/0J117/QRE1113GR.pdf)
# mounted on the BOTTOM layer (facing the floor) along the board's front
# edge -- rev 10 (user request): expanded from 6 to 8 channels, using every
# ADS7830 channel (CH0-CH7, none left NC). Physical pitch is adapted on the
# PCB (board_geom.py / build_pcb.py) to clear the castor wheel footprint --
# see that script for the exact placement, not a fixed pitch here. Read via
# a NEW ADS7830 (TI, TSSOP-16, I2C 8-ch 8-bit ADC, real KiCad symbol
# Analog_DAC:ADS7830) on the SDA/SCL pins freed from the removed
# USER_BTN/USER_BTN2 (see XIAO_NET above) -- keeps this addition to exactly
# 2 GPIO pins regardless of channel count, per user's explicit request to
# multiplex the line sensors over I2C (nRF52840 has no fixed hardware I2C
# pins -- any 2 GPIOs can be firmware-assigned as TWI SDA/SCL). SW1/SW2
# buttons are ALSO back on this same shared bus (see USER INTERFACE section)
# via a separate-address PCF8574 expander -- I2C is a real multi-drop bus,
# so sharing SDA/SCL between the ADC and the GPIO expander is standard.
#
# Per-channel circuit (minimal, no MCU/transistor in the sensing path, per
# user request "keep circuit components minimal and simple"):
#   PLUS3V3 -> 220R -> QRE1113 pin1 (Anode, IR LED) -> pin2 (Cathode) -> GND
#   PLUS3V3 -> indicator LED (anode) -> LED cathode == QRE1113 pin3
#     (Collector) == ADS7830 CHx (via a 1k series/isolation resistor)
#     -> phototransistor -> pin4 (Emitter) -> GND
# The indicator LED IS the pull-up load (no separate pull-up resistor, no
# transistor buffer) -- it lights in direct proportion to reflected IR,
# exactly "driven directly by the receiver based on its sensitivity" as
# specified. QRE1113 datasheet on-state collector current is only
# ~0.1-0.4mA typical at IF=20mA/d=1mm/90%-reflective-white-paper (Fig. 1-3),
# so this indicator is intentionally DIM, not a bright/crisp on-off signal
# -- a deliberate minimal-parts tradeoff, flagged for review/easy upgrade
# (add a transistor stage) if brighter indication is wanted later. IR
# emitter: 220R current-limit resistor (~8.6mA at Vf~1.4V from PLUS3V3),
...
# 2026-07-25) via a PCF8574 8-bit I2C GPIO expander (U5) sharing the SAME
# SDA/SCL bus already wired to the ADS7830 line-sensor ADC (see IR SENSOR
# ARRAY section) -- every XIAO GPIO pin is already spoken for (exact 20/20
# budget), so a shared-bus expander is the only way to add buttons back
# without removing anything else. Added at the END of the file (same reason
# as the buzzer/status-LED sections above): its WIRE() calls must not shift
# the global dogleg lane counter used by every wire earlier in the file --
# confirmed the hard way (placing this block up in the USER INTERFACE header
# section shifted LINE4/5_ADC's bend positions onto each other, an ERC
# "multiple_net_names ... same items" short).
#
# PCF8574 pinout/address table confirmed against TI's own datasheet
# (SCPS068K, DW/N package): A0=A1=A2=GND -> I2C address 0x20 write / 0x21
# read, distinct from the ADS7830's own 0x48/0x49 -- no bus address
# conflict. P0/P1 wired to the 2 buttons' other leg straight to GND: the
# PCF8574 P-ports have a built-in ~100uA weak pull-up current source (TI
# datasheet Electrical Characteristics, IOH row), so NO external pull-up
# resistor is needed -- a direct button-to-GND on P0/P1 is the datasheet's
# own minimal button-input circuit. INT (open-drain interrupt) is left NC --
# firmware polls the expander over I2C rather than using the interrupt line,
# per the same "keep circuits minimal" brief applied elsewhere in this
# design. Real KiCad symbol note: PCF8574T `extends "TCA9534"` (same
# extends-skips-ERC issue as SFH4550/BSS138 elsewhere in this file) --
# TCA9534's base symbol is instantiated directly (identical pin geometry/
# numbering to the real PCF8574, confirmed pin-for-pin against the TI
