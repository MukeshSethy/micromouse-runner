...
| MMSE-PCB-3 | KiCad DRC shall report **0 errors and 0 warnings** (schematic-parity included). | R | M | T | **FAILED** | **32 error-severity violations** at default severity: 18 `pth_inside_courtyard`, 8 `npth_inside_courtyard`, 6 `courtyards_overlap` — all in the dense front sensor cluster + a few R/Q pairs. NOT copper collisions (see MMSE-PCB-2). Root cause + fix: §13 `MMSE-REM-1..4`. |
...
|---|---|---|---|---|---|
| **MMSE-REM-1** | Custom optical/R-Q footprints carry ~0.75 mm courtyard margin → `courtyards_overlap`/`*_inside_courtyard` in the dense front cluster (26 of 32 errors). | Tighten courtyards to IPC-7351 "Least" (body + 0.1–0.15 mm) on TCRT5000 / IR333-A / PT334-6B / R41-42-47-48-49-50 / Q20 / Q27, in the footprint generators (reproducible). | MMSE-PCB-3 | `verify_drc.py` error count drops ~26. | Low (no part moves; IPC-compliant). |
