...
Each net lists its full pin membership as `REF.pin (pin function)`, then the engineering justification. Sources referenced: Toshiba TB6612FNG datasheet, Nexperia HEF4067B datasheet, Espressif ESP32-S3-MINI-1 datasheet + hardware design guidelines, Diodes AP63203 datasheet, the ESP32-S3 datasheet/TRM (LEDC any-GPIO PWM, PCNT quadrature, ADC1-with-WiFi), Pololu QTR-8A documentation, Peter Harrison's micromouse sensor writings, and the project research log (PROJECT_NOTES.md).
...
**Pins:** `A1.10` (A2), `C7.1`, `R4.2`, `R5.1`

Scaled full-pack voltage into ESP32 ADC pin A2 (A1.10). R4 (10k, from the balance connector's pack+ pin J3.1) and R5 (6.2k, to GND) divide 0-8.4V down to 0-3.21V. C7 100nF filters the node, same reasoning as the cell-1 divider. NOTE (flagged risk, also in PROJECT_NOTES): both dividers hang directly on the balance connector, UPSTREAM of switch and fuse -- they drain ~0.52mA + ~0.13mA continuously while the balance lead is plugged, which will deep-discharge a stored pack over some weeks. Mitigation: unplug the balance connector for storage (documented assembly/user rule), or a future revision can add a high-side sense switch.

### `VBAT_CELL1_SENSE`

**Pins:** `A1.11` (A1), `C6.1`, `R2.2`, `R3.1`

Scaled cell-1 voltage into ESP32 ADC pin A1 (A1.11). R2 (10k, from the balance-lead cell-1 tap J3.2) and R3 (22k, to GND) divide 0-4.2V down to 0-2.89V, inside the 3.3V ADC range with margin. C6 100nF across the bottom leg low-passes the node -- the ADC samples a low-impedance-ish source and motor PWM noise is filtered. Firmware computes cell2 = pack - cell1 so BOTH cells are monitored for the per-cell 3.0V LiPo cutoff (a pack reading alone can hide one weak cell). A0-A2 carry all three analog inputs on this board -- confirm the Nano-ESP32 header->GPIO->ADC1-channel mapping in firmware (ADC1, not ADC2, must be used while WiFi is active).
...
**Pins:** `A1.26` (D2), `J10.1` (STBY)

TB6612 standby control (active-low) from ESP32 GPIO D2 (A1.26) to the breakout header J10.1. GPIO-driven rather than strapped high so firmware can hard-disable both H-bridges (fault response, low-power idle) -- an explicit user requirement recorded in PROJECT_NOTES. TB6612 outputs are disabled while STBY is low regardless of the IN/PWM pins, so the robot cannot drive during MCU reset if the pin idles low.

### `PWMA`

**Pins:** `A1.20` (D8), `J10.2` (PWMA)

Motor A speed PWM: ESP32 D8 (A1.20) to the breakout's PWMA (J10.2). The ESP32-S3's LEDC peripheral generates hardware PWM on any GPIO, typically configured 20-25kHz (above audible). Use one LEDC timer for both PWMA and PWMB so the two motors' PWM stays phase-aligned.

### `AIN1`

**Pins:** `A1.22` (D6), `J10.3` (AIN1)

TB6612 motor A direction bit 1 from ESP32 GPIO D6 via A1.22 to the motor-driver breakout control header (J10.3). The IN1/IN2 pair per channel selects forward / reverse / short-brake / stop per the TB6612 truth table while the PWM pin modulates speed. Plain GPIOs suffice -- direction changes are slow, and the ESP32's LEDC PWM works on any pin so no special pin assignment is needed anywhere on the drive side.

### `AIN2`

**Pins:** `A1.23` (D5), `J10.4` (AIN2)

TB6612 motor A direction bit 2 from ESP32 GPIO D5 via A1.23 to the motor-driver breakout control header (J10.4). The IN1/IN2 pair per channel selects forward / reverse / short-brake / stop per the TB6612 truth table while the PWM pin modulates speed. Plain GPIOs suffice -- direction changes are slow, and the ESP32's LEDC PWM works on any pin so no special pin assignment is needed anywhere on the drive side.
...
**Pins:** `A1.21` (D7), `J10.5` (PWMB)

Motor B speed PWM: ESP32 D7 (A1.21) to the breakout's PWMB (J10.5). Same LEDC timer as PWMA, see PWMA rationale.

### `BIN1`

**Pins:** `A1.24` (D4), `J10.6` (BIN1)

TB6612 motor B direction bit 1 from ESP32 GPIO D4 via A1.24 to the motor-driver breakout control header (J10.6). The IN1/IN2 pair per channel selects forward / reverse / short-brake / stop per the TB6612 truth table while the PWM pin modulates speed. Plain GPIOs suffice -- direction changes are slow, and the ESP32's LEDC PWM works on any pin so no special pin assignment is needed anywhere on the drive side.

### `BIN2`

**Pins:** `A1.25` (D3), `J10.7` (BIN2)

TB6612 motor B direction bit 2 from ESP32 GPIO D3 via A1.25 to the motor-driver breakout control header (J10.7). The IN1/IN2 pair per channel selects forward / reverse / short-brake / stop per the TB6612 truth table while the PWM pin modulates speed. Plain GPIOs suffice -- direction changes are slow, and the ESP32's LEDC PWM works on any pin so no special pin assignment is needed anywhere on the drive side.
...
**Pins:** `A1.16` (D12), `J5.5` (ENC_A), `R6.1`

Motor A quadrature encoder phase A: from the N20 motor's Hall encoder via connector J5.5, into ESP32 D12 (A1.16), with 10k pull-up R6 to +3V3. The ESP32-S3 has 4 PCNT (pulse-counter) units with hardware quadrature decoding, and PCNT inputs route through the GPIO matrix from ANY pin -- each motor's phase pair just needs two ordinary GPIOs (no special timer-pin pairing as an STM32 would need). The pull-up is defensive: the encoder's output stage (open-drain vs push-pull) is unverified for the exact unit ordered -- required if open-drain, harmless if push-pull (PROJECT_NOTES).

### `ENC1_B`

**Pins:** `A1.17` (D11), `J5.6` (ENC_B), `R7.1`

Motor A quadrature encoder phase B: from the N20 motor's Hall encoder via connector J5.6, into ESP32 D11 (A1.17), with 10k pull-up R7 to +3V3. The ESP32-S3 has 4 PCNT (pulse-counter) units with hardware quadrature decoding, and PCNT inputs route through the GPIO matrix from ANY pin -- each motor's phase pair just needs two ordinary GPIOs (no special timer-pin pairing as an STM32 would need). The pull-up is defensive: the encoder's output stage (open-drain vs push-pull) is unverified for the exact unit ordered -- required if open-drain, harmless if push-pull (PROJECT_NOTES).

### `ENC2_A`

**Pins:** `A1.18` (D10), `J6.5` (ENC_A), `R8.1`

Motor B quadrature encoder phase A: from the N20 motor's Hall encoder via connector J6.5, into ESP32 D10 (A1.18), with 10k pull-up R8 to +3V3. The ESP32-S3 has 4 PCNT (pulse-counter) units with hardware quadrature decoding, and PCNT inputs route through the GPIO matrix from ANY pin -- each motor's phase pair just needs two ordinary GPIOs (no special timer-pin pairing as an STM32 would need). The pull-up is defensive: the encoder's output stage (open-drain vs push-pull) is unverified for the exact unit ordered -- required if open-drain, harmless if push-pull (PROJECT_NOTES).

### `ENC2_B`

**Pins:** `A1.19` (D9), `J6.6` (ENC_B), `R9.1`

Motor B quadrature encoder phase B: from the N20 motor's Hall encoder via connector J6.6, into ESP32 D9 (A1.19), with 10k pull-up R9 to +3V3. The ESP32-S3 has 4 PCNT (pulse-counter) units with hardware quadrature decoding, and PCNT inputs route through the GPIO matrix from ANY pin -- each motor's phase pair just needs two ordinary GPIOs (no special timer-pin pairing as an STM32 would need). The pull-up is defensive: the encoder's output stage (open-drain vs push-pull) is unverified for the exact unit ordered -- required if open-drain, harmless if push-pull (PROJECT_NOTES).
...
**Pins:** `A1.15` (D13), `U4.10` (S0), `U5.10` (S0)

Mux channel-select bit 0 (binary weight 1), ESP32 GPIO D13 via A1.15 driving select input S0 on BOTH HEF4067s in parallel (U4.10 and U5.10). Sharing one 4-bit select bus across the read-mux and the write-demux is the core of the sensor architecture: selecting channel N simultaneously picks sensor N's phototransistor for reading AND sensor N's LED driver for pulsing, enforcing one-emitter-one-receiver lockstep (crosstalk avoidance) while spending only 4 GPIOs + 1 ADC pin + 1 pulse pin on 14 sensors. Channel index = S3 S2 S1 S0 binary; WALL1..6 = channels 0-5 (Y0-Y5), LINE1..8 = channels 6-13 (Y6-Y13).

### `MUX_S1`

**Pins:** `A1.9` (A3), `U4.11` (S1), `U5.11` (S1)

Mux channel-select bit 1 (binary weight 2), ESP32 GPIO A3 via A1.9 driving select input S1 on BOTH HEF4067s in parallel (U4.11 and U5.11). Sharing one 4-bit select bus across the read-mux and the write-demux is the core of the sensor architecture: selecting channel N simultaneously picks sensor N's phototransistor for reading AND sensor N's LED driver for pulsing, enforcing one-emitter-one-receiver lockstep (crosstalk avoidance) while spending only 4 GPIOs + 1 ADC pin + 1 pulse pin on 14 sensors. Channel index = S3 S2 S1 S0 binary; WALL1..6 = channels 0-5 (Y0-Y5), LINE1..8 = channels 6-13 (Y6-Y13).

### `MUX_S2`

**Pins:** `A1.8` (A4), `U4.14` (S2), `U5.14` (S2)

Mux channel-select bit 2 (binary weight 4), ESP32 GPIO A4 via A1.8 driving select input S2 on BOTH HEF4067s in parallel (U4.14 and U5.14). Sharing one 4-bit select bus across the read-mux and the write-demux is the core of the sensor architecture: selecting channel N simultaneously picks sensor N's phototransistor for reading AND sensor N's LED driver for pulsing, enforcing one-emitter-one-receiver lockstep (crosstalk avoidance) while spending only 4 GPIOs + 1 ADC pin + 1 pulse pin on 14 sensors. Channel index = S3 S2 S1 S0 binary; WALL1..6 = channels 0-5 (Y0-Y5), LINE1..8 = channels 6-13 (Y6-Y13).

### `MUX_S3`

**Pins:** `A1.7` (A5), `U4.13` (S3), `U5.13` (S3)

Mux channel-select bit 3 (binary weight 8), ESP32 GPIO A5 via A1.7 driving select input S3 on BOTH HEF4067s in parallel (U4.13 and U5.13). Sharing one 4-bit select bus across the read-mux and the write-demux is the core of the sensor architecture: selecting channel N simultaneously picks sensor N's phototransistor for reading AND sensor N's LED driver for pulsing, enforcing one-emitter-one-receiver lockstep (crosstalk avoidance) while spending only 4 GPIOs + 1 ADC pin + 1 pulse pin on 14 sensors. Channel index = S3 S2 S1 S0 binary; WALL1..6 = channels 0-5 (Y0-Y5), LINE1..8 = channels 6-13 (Y6-Y13).

### `MUX_SENSE`

**Pins:** `A1.12` (A0), `U4.1` (Z)

Analog common (Z, U4.1) of the READ mux into ESP32 ADC pin A0 (A1.12). Whatever channel S3..S0 selects, that sensor's phototransistor divider node appears here. One ADC pin thus reads all 14 sensors -- far fewer than 14 ADC-capable pins exist on the Nano header, so muxing is mandatory; it also saves GPIO for the drive side. HEF4067 is a bidirectional ANALOG switch (verified: Nexperia datasheet Rev.11 calls it a bidirectional transmission-gate switch), so the divider voltage passes through unmodified in DC terms. Verified caveats for firmware (adversarial datasheet check, 2 independent agents): the datasheet only characterizes Ron at 5/10/15V (350R typ / 2500R MAX at 5V); at our 3.3V, mid-scale Ron rises to ~1k+ typical and plausibly several k worst case (metal-gate CMOS, only ~1.65V overdrive at mid-supply). Into the ADC's high-Z input this costs NO ratio accuracy (no DC current flows), but the 47k pull-up + ~1k+ Ron source impedance is far above the ESP32 ADC's fast-sample limit, so USE A LONG ADC SAMPLING TIME (or an op-amp buffer if speed matters). Channel off-leakage (spec'd 1000nA max Z-port at 25C) through 47k is a low-tens-of-mV worst-case offset, typically negligible.

### `LED_PULSE`

**Pins:** `A1.6` (A6), `U5.1` (Z)

ESP32 GPIO A6 (A1.6) into the WRITE demux common (Z, U5.1). Firmware raises this for ~60-100us to fire the currently-selected sensor's IR LED via its BSS138 gate, samples MUX_SENSE ('bright'), lowers it, samples again ('ambient'), and subtracts -- synchronous detection that cancels ambient IR (Peter Harrison's method, see PROJECT_NOTES). FIRMWARE RULE (flagged): unselected demux outputs are high-impedance, so a BSS138 gate holds its last charge when deselected. ALWAYS return LED_PULSE low while the channel is still selected (gate discharges through the mux), and on boot walk all 14 channels with LED_PULSE low to discharge any power-up gate charge. Otherwise a deselected LED can stay on -- crosstalk + battery drain.
...
**Pins:** `A1.5` (A7), `R10.2`, `SW1.1`

Start-run button: ESP32 A7 (A1.5), pulled to +3V3 through R10 10k, switched to GND by SW1 (active-low). Standard micromouse UX -- arm the run without touching the robot's power.
...
| `A1.1` | VIN | VIN -- the dev board is fed regulated 3.3V directly on its 3V3 pin; feeding VIN too would run its onboard regulator in parallel with ours. |

| `A1.13` | AREF | AREF -- the ESP32's ADC uses its internal reference; nothing external supplied. |

| `A1.28` | RESET | RESET (digital-row copy) -- same line as A1.3, see there. |

| `A1.29` | D0 | D0/RX -- kept free for USB-serial debug on the dev board; no on-board UART link exists now that the ESP32 is the sole controller. |

| `A1.3` | RESET | RESET (analog-row copy) -- nothing on this board needs to hard-reset the ESP32; its own button/auto-reset handle it. |

| `A1.30` | D1 | D1/TX -- kept free, same reason as D0/RX. |

| `A1.4` | 5V | 5V -- the board is battery-powered at 3.3V throughout; the dev board's 5V USB rail is only alive while its USB is plugged. |
