...
## 2026-07-16: rev 5.2 -- orderable BOM, complete STEP, scripted fab outputs

The three deferred audit items, all gated now:

**BOM (pcb/BOM.csv, regenerated by export_fab.py):** 42 line items, 100%
MPN coverage. Every part number adversarially verified against manufacturer
datasheets + live distributor listings (two research passes, 31 parts):
packages checked against the design footprints, every SOT-23 FET checked for
pin1=G/2=S/3=D. Notable calls:
- USBLC6-2P6 would have been WRONG (SOT-666); the SOT-23-6 part is
  USBLC6-2SC6.
- Module pinned to ESP32-S3-WROOM-1-N16: octal-PSRAM (-R8*/-R16V) variants
  consume IO35-37, which this design uses for buttons B/C + VBUS sense.
- L1 = SRP7028A-1R5M (1.5uH datasheet-typical for TPS63001; 18A Isat).
- Line array: IR11-21C/TR8 emitter + PT11-21B/L41/TR8 phototransistor
  (both true 1206). The PT's BLACK daylight filter is REQUIRED (unfiltered
  -C re-opens optical feedback from the red indicators). The B-suffix is a
  real Everlight MPN (datasheet DPT-0000132) but big distributors stock only
  the -C: source B from Everlight direct/brokers, or fall back to ams-OSRAM
  SFH 320 FA (PLCC-2, fits the 1206 land per the rev-4 note).
- SOURCING ALARM: ams-OSRAM SFH 309 FA (wall phototransistors) is LAST TIME
  BUY, deadline 2026-12-01 -- order lifetime quantity NOW. SFH 4550 is on
  the same THT-opto phase-out track (PDN 2025-160); buy ahead.
- TB6612FNG body is 5.6mm vs the KiCad SSOP-24 footprint's 5.3mm drawing --
  same 0.65 pitch and lead span, solderable as-is; flagged for review.
- Latent value-plumbing bug fixed: photo_val/led_val for the SMD line parts
  were computed but never passed, so D7-14/Q8-15 shipped VALUE-labeled as
  THT parts. Helpers take a value override now; 147 board Value fields
  re-synced from the netlist (cosmetic; net topology proven identical).

**3D/STEP (gen_step_models.py):** hand-authored AP214 box-solid models for
N20 motors (7 solids, true Pololu dims), U1 DRC0010J and L1 SRP7028A -- the
KiCad install ships NO .step (or .wrl) for the latter two, and VRML-only
models are skipped by STEP export. U1/L1 model paths repointed to
${KIPRJMOD}/3d/ (text-level post-save patch in build_pcb: pcbnew's
fp.Models() returns a SWIG copy, in-place edits vanish). --subst-models
picks the sibling .step next to each .wrl. Export now loads EVERY model.

**Fab outputs (export_fab.py, outputs gitignored under pcb/fab/):** gerbers
(11 layers) + Excellon drill + PDF maps + pos CSV + fit-check STEP + BOM,
